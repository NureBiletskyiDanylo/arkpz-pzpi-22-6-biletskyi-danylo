...
public class WarehouseController(IWarehouseRepository warehouseRepository) : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateWarehouse([FromBody] WarehouseCreateModel model, int id)
    {
        ...
        LogStorageRequirementChange(model, oldWarehouse, email);
    ...
    private void LogStorageRequirementChange(WarehouseCreateModel newModel, Warehouse oldModel, string emailOfChanger)
    {
        if ((newModel.MaxTemperature != oldModel.MaxTemperature) ||
            (newModel.MinTemperature != oldModel.MaxTemperature) ||
            (newModel.MaxHumidity != oldModel.MaxHumidity) ||
            (newModel.MinHumidity != oldModel.MinHumidity))
        {
            Log.Information(
                ...
                oldModel.Name,
                emailOfChanger,
                oldModel.MaxTemperature,
                oldModel.MinTemperature,
                oldModel.MaxHumidity,
                oldModel.MinHumidity,
                newModel.MaxTemperature,
                newModel.MinTemperature,
                newModel.MaxHumidity,
                newModel.MinHumidity);
